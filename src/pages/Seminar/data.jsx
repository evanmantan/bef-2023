import SANDIAGA from "../../assets/seminar/sandiaga.webp";
import IVAN from "../../assets/seminar/ivan.webp";
import AHMAD from "../../assets/seminar/ahmad.webp";
import ARDANTYA from "../../assets/seminar/ardantya.webp";
import ARSJAD from "../../assets/seminar/arsjad.webp";
import RENDY from "../../assets/seminar/rendy.webp";
import VIRINGGA from "../../assets/seminar/viringga.webp";

export const keynoteSpeech = {
  image: SANDIAGA,
  name: "Sandiaga Salahuddin Uno",
  position: "Menteri Pariwisata dan Ekonomi Kreatif Republik Indonesia",
};

export const seminarDay1 = [
  {
    image: ARSJAD,
    name: "Arsjad Rasjid",
    position: "Indonesian Entrepreneur",
    title:
      "Sesi 1 : How The Creative Entrepreneur Can Contribute to Indonesia?",
    description:
      "Memahami peran penting kreativitas dalam perekonomian Indonesia dan bagaimana mereka sebagai wirausaha kreatif dapat berkontribusi dalam pengembangan industri kreatif di negara ini.",
  },
  {
    image: ARDANTYA,
    name: "Ardantya Syahreza",
    position: "Chairman of Malang Health Tourism Board",
    title: "Sesi 2 : How To Act As an Entrepreneur?",
    description:
      "Mengidentifikasi keterampilan dan sikap yang diperlukan untuk menjadi seorang wirausaha yang sukses, termasuk kreativitas, kepemimpinan, ketahanan, dan kemampuan beradaptasi dalam lingkungan bisnis yang berubah-ubah.",
  },
  {
    image: AHMAD,
    name: "Ahmad Syaifullah M.T.I",
    position: "CTO Chickin",
    title: "Sesi 3 : How To Build My First Business : Business Model",
    description:
      "Mempelajari konsep dasar pembuatan model bisnis, termasuk pemilihan segmen pasar, nilai proposisi, sumber pendapatan, dan saluran distribusi, serta bagaimana menerapkannya dalam perencanaan bisnis mereka untuk memulai usaha pertama mereka.",
  },
];

export const seminarDay2Session1 = {
  title: "Sesi 1 : Digital Platforms and Market Access for Creative Industries",
  description:
    "Memahami potensi platform digital dalam memperluas jangkauan dan akses pasar bagi industri kreatif, dan belajar bagaimana menggunakan platform digital dengan efektif untuk mempromosikan dan menjual produk atau layanan kreatif mereka.",
  speakers: [
    {
      image: IVAN,
      name: "Ivan Halim",
      position: "CMO Sevenpreneur",
    },
    {
      image: RENDY,
      name: "Rendy Sudigja",
      position: "Founder Zaafer Indonesia",
    },
  ],
};

export const seminarDay2Session2 = {
  image: VIRINGGA,
  name: "Viringga Kusuma",
  position: "CEO AMATI Indonesia",
  title: "Sesi 2 : Eco-Entrepreneurship: Building Sustainable Startups",
  description:
    "Mengidentifikasi peluang bisnis berkelanjutan dalam konteks lingkungan, belajar tentang praktik bisnis hijau, dan memahami bagaimana mendirikan startup yang berfokus pada keberlanjutan dengan dampak positif pada lingkungan dan masyarakat.",
};
